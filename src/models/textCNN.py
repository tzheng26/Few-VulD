...
from tensorflow.compat.v1.keras.layers import Input, Dense, Embedding, Conv2D, MaxPool2D, Reshape, Concatenate,Flatten,Dropout
...
class textCNN(object):
    ...
    def buildModel(self, word_index, embedding_matrix):
        ...
        reshape = Reshape((self.MAX_LEN, self.EMBEDDING_DIM, 1))(sharable_embedding)
        conv_0 = Conv2D(num_filters, kernel_size=(filter_sizes[0], self.EMBEDDING_DIM), padding='valid', kernel_initializer=initializers.TruncatedNormal(mean=0.0, stddev=0.1),
        ...
        conv_1 = Conv2D(num_filters, kernel_size=(filter_sizes[1], self.EMBEDDING_DIM), padding='valid', kernel_initializer=initializers.TruncatedNormal(mean=0.0, stddev=0.1),
                            bias_initializer=initializers.constant(value=0.1), activation='relu')(reshape)
        maxpool_1 = MaxPool2D(pool_size=(self.MAX_LEN - filter_sizes[1] + 1, 1), strides=(1,1), padding='valid')(conv_1)
        
        conv_2 = Conv2D(num_filters, kernel_size=(filter_sizes[2], self.EMBEDDING_DIM), padding='valid', kernel_initializer=initializers.TruncatedNormal(mean=0.0, stddev=0.1),
                            bias_initializer=initializers.constant(value=0.1), activation='relu')(reshape)
        maxpool_2 = MaxPool2D(pool_size=(self.MAX_LEN - filter_sizes[2] + 1, 1), strides=(1,1), padding='valid')(conv_2)
        
        conv_3 = Conv2D(num_filters, kernel_size=(filter_sizes[3], self.EMBEDDING_DIM), padding='valid', kernel_initializer=initializers.TruncatedNormal(mean=0.0, stddev=0.1),
                            bias_initializer=initializers.constant(value=0.1), activation='relu')(reshape)
